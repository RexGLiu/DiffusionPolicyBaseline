...
from diffusion_policy.gym_util.obs_noise_wrapper import ObsNoiseWrapper
from diffusion_policy.gym_util.env_tools import get_action_space_bounds
...
class KitchenLowdimRunner(BaseLowdimRunner):
    def __init__(self,
        ...
        def env_fn():
            ...
            if o_noise is not None:
                wrapped_env = ObsNoiseWrapper(wrapped_env, o_noise, noise_seed)
        ...
        for i in range(n_train):
            ...
            def init_fn(env, init_qpos=init_qpos, init_qvel=init_qvel, enable_render=enable_render):
                ...
                # setup rendering
                # video_wrapper
                video_env = env.env if o_noise is None else env.env.env
                assert isinstance(video_env, VideoRecordingWrapper)
                ...
                kitchen_env = video_env.env
                assert isinstance(kitchen_env, KitchenLowdimWrapper)
                kitchen_env.init_qpos = init_qpos
                kitchen_env.init_qvel = init_qvel
        ...
        for i in range(n_test):
            ...
            def init_fn(env, seed=seed, enable_render=enable_render):
                ...
                kitchen_env = video_env.env
                assert isinstance(kitchen_env, KitchenLowdimWrapper)
                kitchen_env.init_qpos = None
                kitchen_env.init_qvel = None
        ...
        self.a_space_low, self.a_space_high = get_action_space_bounds(env.action_space)
        self.np_rng_noise = np.random.default_rng(noise_seed)
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                if self.a_noise is not None:
                    action += self.a_noise * self.np_rng_noise.normal(size=action.shape)
                    action = np.clip(action, self.a_space_low, self.a_space_high)

...
# Initialisation function taken from https://github.com/facebookresearch/gen_dgrl/tree/main/procgen.
...
class ConvEncoder(ModuleAttrMixin):
    def __init__(self, observation_space, embed_dim, 
                 channels=[32,64,64], fc_dim=1024, kernels=[8,4,3], strides=[4,2,1]):
        """
            observation_space: (H, W, C)
            embed_dim: int
            fc_dim: int
            channels: list of channels, e.g. [32, 64, 64]
            kernels: list of kernel sizes, e.g. [8, 4, 3]
            strides: list of strides,      e.g. [4, 2, 1]
        """
        super(ConvEncoder, self).__init__()

        assert len(channels) == len(kernels) == len(strides) == 3, f"Number of channels, kernels and strides must be 3. Received {channels} channels, {kernels} kernels, and {strides} strides."
...
class BasicBlock(ModuleAttrMixin):
    """
    ...
    def __init__(self, n_channels):
        ...
        self.conv1 = Conv2d_tf(n_channels, n_channels, kernel_size=3, stride=1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = Conv2d_tf(n_channels, n_channels, kernel_size=3, stride=1)
...
class ResNetEncoderBlock(ModuleAttrMixin):
    def __init__(self, in_channels, out_channels):
        ...
        self.layer3 = BasicBlock(out_channels)
        self.layer4 = BasicBlock(out_channels)
...
class ResNet20Encoder(ModuleAttrMixin):
    """
...
class TransformerWithEncoderForDiffusion(ModuleAttrMixin):
    def __init__(self,
            obs_encoder: ModuleAttrMixin,
    ...
    def encode_obs(self, 
        ...
        sample = sample.reshape(B*T,C,H,W)  # fold T into the batch dim

        observations = self.obs_encoder(sample)

        assert len(observations.shape) == 2
        observations = observations.reshape(B,T,-1)   # unfold T from batch dim
    ...
    def forward(self, 
        sample: torch.Tensor, 
        timestep: Union[torch.Tensor, float, int], 
        cond: torch.Tensor, **kwargs) -> torch.Tensor:
        """
        sample: (B,T,input_dim)
        timestep: (B,) or int, diffusion step
        cond: (B,T',C,H,W)
        ...
        cond = self.encode_obs(cond)
        actions = self.transformer(sample, timestep, cond)
    ...
    def get_optim_groups(self, weight_decay: float=1e-3):
        ...
        blacklist_weight_modules = (torch.nn.LayerNorm, torch.nn.Embedding, torch.nn.MaxPool2d)
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                fpn = "%s.%s" % (mn, pn) if mn else pn  # full param name
